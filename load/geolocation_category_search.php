...
$categories = fRecordSet::buildFromSQL(
	...
	"SELECT * FROM economic_unit_categories $where (economic_unit_category_name LIKE '%$query%' OR economic_unit_category_region IN (SELECT id_region FROM region WHERE name LIKE '%$query%')) LIMIT $start,$limit",
	"SELECT count(*) FROM economic_unit_categories $where (economic_unit_category_name LIKE '%$query%' OR economic_unit_category_region IN (SELECT id_region FROM region WHERE name LIKE '%$query%'))",
	$limit, // $limit
	$page  // $page
);	
